            ...
            <div className="max-w-7xl   mx-auto flex justify-between items-center px-5 md:px-10 py-5 ">
                <Link href={"/"} className="flex gap-2 items-center">
                    <div className="bg-(--primary) p-2 rounded-full">
                        <LuMountainSnow className=" text-[28px] text-white" />
                    </div>
                    <span className="font-bold text-xl">Bukit Waruwangi</span>
                </Link>

                <ul className= {`${bgNavbar} ${sliderMenu} border-t border-(--border) backdrop-blur duration-400 absolute top-21 left-0 w-full p-5 md:static md:border-none md:backdrop-blur-none md:bg-transparent md:w-auto md:p-0 flex-col md:flex-row gap-7`}>
                    {navList.map((navItem =>{
                        const isActive = Pathname === navItem.href;
                        return (
                            <li key={navItem.name} className={`${liHover} ${isActive ? "text-(--primary)" : ""}`}>
                ...
                <div className="md:hidden text-4xl">
                    {toggle ?  <IoMdClose onClick={() => setToggle(!toggle)}/> : <HiOutlineMenu onClick={() => setToggle(!toggle)}/>}
                </div>
            </div>
        </nav>
    );
